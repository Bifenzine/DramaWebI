...
import {  getSeriesCredits, getSeriesDetails} from '../../data/FetchingDataAxios';
...
const SeriesCastAndCrews = () => {
    const { id } = useParams();
    

    const [castsSeries, setCastsSeries] = useState(null)
    const [crewsSeries, setCrewsSeries] = useState([])

    useEffect(() => {
        getSeriesDetails(id)
            .then((response) => {
                // console.log(response.data)
                setCastsSeries(response.data.credits.cast);
               
                setCrewsSeries(response.data.credits.crew);
            })
            .catch((error) => {
...
export default SeriesCastAndCrews;